...
const MyAppointments = () => {
    ...
    const slotDateFormat = (slotDate) => {
        if (!slotDate) return "Invalid Date"; // Safety check
            ...
            <div className=''>
                {appointments.map((item, index) => (
                    <div key={index} className='grid grid-cols-[1fr_2fr] gap-4 sm:flex sm:gap-6 py-4 border-b'>
                        <div>
                            {/* SAFE CHECK: Check if docData exists before trying to access image */}
                            <img className='w-36 bg-[#EAEFFF]' src={item.docData?.image || assets.doc_avatar} alt="" />
                        </div>
                        <div className='flex-1 text-sm text-[#5E5E5E]'>
                            {/* SAFE CHECK: Use ?. for all docData properties */}
                            <p className='text-[#262626] text-base font-semibold'>{item.docData?.name || "Doctor Unavailable"}</p>
                            <p>{item.docData?.specialization || item.docData?.speciality || "General"}</p>
                            
                            <p className='text-[#464646] font-medium mt-1'>Address:</p>
                            {/* SAFE CHECK: Address might be missing if doctor was deleted */}
                            <p className=''>{item.docData?.address?.line1 || "N/A"}</p>
